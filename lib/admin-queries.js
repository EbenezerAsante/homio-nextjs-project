...
// Upload a video file (max 3 minutes, enforced client-side before calling this)
// and store its public URL on the listing's video_url column.
export async function uploadListingVideo(agentId, listingId, file) {
  const supabase = createClient();
  const path = `${agentId}/${listingId}/${Date.now()}-${file.name}`;

  const { error: uploadErr } = await supabase.storage.from("listing-videos").upload(path, file);
  if (uploadErr) throw uploadErr;

  const { data: publicUrlData } = supabase.storage.from("listing-videos").getPublicUrl(path);
  const publicUrl = publicUrlData.publicUrl;

  const { error: dbErr } = await supabase.from("listings").update({ video_url: publicUrl }).eq("id", listingId);
  if (dbErr) throw dbErr;

  return publicUrl;
}

// Save an external video link (e.g. YouTube) instead of an uploaded file
