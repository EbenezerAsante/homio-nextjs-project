...
// Universal inbox — every conversation this user is part of, whether
// they're the buyer or the assigned agent on it. Powers the bottom-nav
// Messages tab, which every user sees regardless of role. The agent's
// Customer Inbox (fetchAgentConversationsV2) is a filtered subset of
// this same data — conversations table, two views.
// Lightweight total unread count across every conversation this user is
// part of (buyer or agent side) — powers the bottom-nav Messages badge.
// Deliberately a separate, cheap query rather than reusing
// fetchAllUserConversationsV2, since the navbar needs this on every page
// load / poll, not just when the Messages page itself is open.
...
export async function fetchAllUserConversationsV2(userId) {
  const supabase = createClient();
  const { data: conversations, error } = await supabase
    .from("conversations")
    .select(
      "*, listings(id, title, status, price, city, region, listing_images(url, sort_order), agents(full_name)), profiles!conversations_buyer_id_fkey(full_name)"
    )
    .or(`buyer_id.eq.${userId},agent_id.eq.${userId}`)
    .order("last_message_at", { ascending: false, nullsFirst: false });
  if (error) {
    console.error("fetchAllUserConversationsV2 error:", error.message);
    return [];
  }

  const withRole = (conversations || []).map((c) => ({
    ...c,
    myRole: c.buyer_id === userId ? "buyer" : "agent",
    // Who the current user is talking to, for display purposes.
    counterpartName: c.buyer_id === userId
      ? (c.listings?.agents?.full_name || "the agent")
      : (c.profiles?.full_name || "A buyer"),
  }));

  return attachLastMessageAndUnread(supabase, withRole, userId);
}

// Platform admin: every conversation on the platform, for the Message
// Center. Relies on the existing blanket admin RLS policies (already in
// place) rather than needing anything new — is_platform_admin() already
// grants read access to conversations, messages, and profiles.
export async function fetchAllConversationsAdmin() {
  const supabase = createClient();
  const { data: conversations, error } = await supabase
    .from("conversations")
    .select(
      "*, listings(id, title, status, listing_images(url, sort_order)), buyer:profiles!conversations_buyer_id_fkey(full_name, email), agent_profile:profiles!conversations_agent_id_fkey(full_name, email)"
    )
    .order("last_message_at", { ascending: false, nullsFirst: false });
  if (error) {
    console.error("fetchAllConversationsAdmin error:", error.message);
    return [];
  }
  ...
  return conversations.map((c) => {
    ...
    return { ...c, lastMessage: msgs.length ? msgs[msgs.length - 1] : null, messageCount: msgs.length };
  });
}
