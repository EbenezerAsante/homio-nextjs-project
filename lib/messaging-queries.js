...
export async function fetchBuyerConversations(buyerId) {
  ...
  // One batched query for every message across all conversations, then
  // group client-side — avoids an N+1 query per conversation row.
  const ids = enquiries.map((e) => e.id);
  const { data: allMessages } = await supabase
    .from("messages")
    .select("enquiry_id, body, created_at, sender_id, read_at")
    .in("enquiry_id", ids)
    .order("created_at", { ascending: true });

  const byEnquiry = {};
  (allMessages || []).forEach((m) => {
    if (!byEnquiry[m.enquiry_id]) byEnquiry[m.enquiry_id] = [];
    byEnquiry[m.enquiry_id].push(m);
  });

  return enquiries.map((e) => {
    const msgs = byEnquiry[e.id] || [];
    const last = msgs.length ? msgs[msgs.length - 1] : null;
    const unreadCount = msgs.filter((m) => m.sender_id !== buyerId && !m.read_at).length;
    return { ...e, lastMessage: last, unreadCount };
  });
}
